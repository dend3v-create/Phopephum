// ============================================================
// ยามราหูค้นทรัพย์ — Complete TypeScript Database
// Ready-to-use in Next.js / Expo / Node.js
// ============================================================
...
// ── SUB BLOCKS (9 ยามย่อย 10 นาที) ─────────────────────────
export const SUB_BLOCKS: SubBlock[] = [
...
// ── YAM RULES (คำทำนาย 4 มิติ) ──────────────────────────────
export const YAM_RULES: YamRule[] = [
  {
...
// ── YAM MATRIX (7 วัน × 16 รอบ = 112 rows) ─────────────────
// format: [day, block_id, yam_number]
// day: 1=อาทิตย์ 2=จันทร์ 3=อังคาร 4=พุธ 5=พฤหัส 6=ศุกร์ 7=เสาร์
...
export const YAM_MATRIX: YamMatrix[] = _raw.map(([d, b, y], i) => ({
...
function findTimeBlock(totalMinutes: number): TimeBlock | null {
  // normalize: minutes after midnight get +1440 for night blocks spanning midnight
  for (const tb of TIME_BLOCKS) {
    let start = timeToMinutes(tb.start_time);
    let end   = timeToMinutes(tb.end_time);
    if (tb.id === 12) end = 24 * 60; // 22:30-00:00 → end = 1440
    if (end === 0) end = 1440;
...
export function calculateRahu(date: Date): RahuResult | null {
  // JS getDay(): 0=Sun...6=Sat → our day_of_week: 1=Sun...7=Sat
  ...
  // minutes elapsed since block start
  ...
  const minutesElapsed = elapsed % 90; // safety mod

  // find sub-block
  const subBlock = SUB_BLOCKS.find(
    sb => minutesElapsed >= sb.minute_start && minutesElapsed < sb.minute_end
  ) ?? SUB_BLOCKS[8];

  // find yam number from matrix
  const matrixRow = YAM_MATRIX.find(
    m => m.day_of_week === dayOfWeek && m.time_block_id === mainBlock.id
  );
  if (!matrixRow) return null;

  const yamRule = YAM_RULES.find(r => r.yam_number === matrixRow.yam_number)!;
...
// console.log(calculateRahu(new Date()));
