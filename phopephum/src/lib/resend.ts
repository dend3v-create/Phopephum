/**
 * resend.ts
 * Email client สำหรับ Phopephum (Welcome, Report, Alert)
 */
...
const FROM = process.env.RESEND_FROM ?? 'noreply@phopephum.com'
...
export async function sendWelcomeEmail(to: string, name: string) {
  ...
  return resend.emails.send({
    from: FROM,
    to,
    subject: `ยินดีต้อนรับสู่ Phopephum, คุณ${name}! 🌟`,
    html: `
      <div style="font-family:'Sarabun',sans-serif;background:#0A0806;color:#F5F0E8;padding:40px;border-radius:12px;max-width:600px;margin:0 auto">
        <h1 style="color:#C9A96E;font-size:24px;margin-bottom:16px">ยินดีต้อนรับสู่ Phopephum ✨</h1>
        <p style="font-size:16px;line-height:1.7">สวัสดีคุณ<strong style="color:#C9A96E">${name}</strong>,</p>
        <p style="font-size:16px;line-height:1.7">ขอบคุณที่เลือกใช้บริการ <strong>Phopephum</strong> — แพลตฟอร์มพยากรณ์ยามอัฐกาลและดวงชะตาชีวิตระดับพรีเมียม</p>
        ...
        <p style="font-size:13px;color:#8B7E6E;margin-top:32px">© ${new Date().getFullYear()} Phopephum — ระบบดวงชะตาและยามอัฐกาล</p>
      </div>
    `,
  })
}
...
export async function sendContactEmail(
  ...
  return resend.emails.send({
    ...
    replyTo: email,
    subject: `[Phopephum] ข้อความจาก ${name}`,
