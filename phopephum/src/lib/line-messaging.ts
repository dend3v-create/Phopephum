/**
 * line-messaging.ts
 * LINE Messaging API Client (Flex Messages) สำหรับ Phopephum
...
export async function notifyAdminNewUser(
  ...
  await sendLineFlexMessage(
    `📥 [Phopephum] มีข้อความติดต่อใหม่จากคุณ ${name}`,
    flexContents
  )
}
...
export async function notifyAdminUpgrade(
  ...
  await sendLineFlexMessage(
    `💎 [Phopephum] ขอบพระคุณ! คุณ ${name} ได้อัปเกรดเป็นแพ็กเกจ ${plan} สำเร็จ`,
    flexContents
  )
}
