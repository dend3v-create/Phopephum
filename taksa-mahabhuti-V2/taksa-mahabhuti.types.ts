 ...
 *   - เพิ่ม TaksaMahaInput.checkDate — ใช้วันที่จริงแทนแค่ปี
 *
...
/**
 * ลำดับดาวทักษา (8 ดวง) — ใช้สำหรับ:
 *   - ทักษากำเนิด (นับวน 8 ภพ)
 *   - ทักษาจรรายปี  (modulo 8)
 *   - ดาวเสวยอายุ   (กำลังดาวรวม 108 ปี)
 *
 * ลำดับ: อาทิตย์→จันทร์→อังคาร→พุธ→เสาร์→พฤหัส→ราหู→ศุกร์
 */
 ...
 * หมายเหตุ: มี 8 ภพ 8 ดาว → ดาวทุกดวงมีภพ 1:1 พอดี
 */
...
export interface SawaiResult {
  ...
  /** ช่วงอายุย่างของวัยเสวยอายุ */
  sawaiAgeStart: number;
  sawaiAgeEnd: number;
  /** วันเริ่ม/สิ้นสุดวัยเสวยอายุ */
  sawaiDateStart: Date;
  sawaiDateEnd: Date;
  ...
  subStar: StarNumber;
  subStarName: string;
  /** วันเริ่ม/สิ้นสุดช่วงดาวแทรก */
  subDateStart: Date;
  subDateEnd: Date;
  /** ระยะเวลาดาวแทรกเป็นวัน */
...
/**
 * ลำดับเดิน Sequence มหาภูติ
 * เริ่มจากโลกาวินาศ วนตามลำดับนี้
 */
...
export interface TaksaMahaInput {
   ...
   * ใช้หา dayOfWeek → StarNumber และคำนวณ ageYang แม่นยำ
   */
  birthDate: Date;
  /**
   * วันที่ต้องการตรวจดวง (JavaScript Date, Gregorian)
   * ใช้คำนวณ ageYang แบบแม่นยำ (ตัดรอบวันเกิด)
   * และใช้หาดาวแทรก (sub-period) ระดับวัน
   */
  checkDate: Date;
  /**
   * จุลศักราชปีเกิด (csNatal)
   * สูตร: พ.ศ. - 1181 = จ.ศ.
   * ตัวอย่าง: เกิดปี 2525 → csNatal = 1344
   * หมายเหตุ: บางตำราใช้ปี จ.ศ. ของระบบโหราศาสตร์แทน ให้ตรวจสอบกับตำราเสมอ
   */
  csNatal: number;
  /**
   * จุลศักราชปีที่ตรวจดวง (csTransit)
   * ตัวอย่าง: ปี 2569 → csTransit = 1388
   * คำนวณอัตโนมัติได้ด้วย buddhToCS(checkDate.getFullYear() + 543)
   */
  csTransit: number;
}
