-- 022_payout_and_admin_operations.sql
-- ==============================================================================
-- 🏛️ PHOPEPHUM V3 — STEP 6.5.6: PARTNER PAYOUT & ADMIN OPERATIONS
-- ==============================================================================
-- Strict State Machine, Monitored Clearance Telemetry, Failed/Partial Job Logs,
-- Immutable Admin Financial Audit Trail, and Retry-Safe RPCs
-- ==============================================================================

-- 1. ตารางบันทึกผลการรัน Job การเงิน (Financial Job Telemetry & Monitoring)
...
-- 2. ตารางบันทึก Immutable Audit Trail สำหรับการกระทำของ Admin ฝั่งการเงิน
CREATE TABLE IF NOT EXISTS public.admin_financial_audit_logs (
    ...
    action TEXT NOT NULL, -- 'approve_payout' | 'reject_payout' | 'trigger_transfer' | 'force_retry_job' | 'void_transfer' | 'manual_reconcile'
...
-- 4. Enhanced Atomic RPC: Monitored Holding Clearance with Telemetry & Failure Details
...
BEGIN
    ...
    LOOP
        BEGIN
            ...
            -- 1. อัปเดต Partner Entity (3 Operational Balances + 1 Clawback Debt)
            ...
            -- 3. หากมี Clawback Offset บันทึก Entry พิเศษ
...
-- 5. Enhanced Atomic RPC: Strict Admin Payout State Machine with Immutable Audit Log
...
BEGIN
    ...
    -- approved -> processing
    -- approved -> rejected (Before transfer execution)
    -- processing -> completed
    -- processing -> failed (Omise transfer error)
    -- failed -> processing (Retry transfer)
    -- failed -> rejected (Cancel & return funds)
